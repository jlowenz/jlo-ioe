/*
 * $Id: JXMultiSplitPane.java,v 1.2 2007/03/16 21:38:04 rbair Exp $
 *
...
import jlo.ioe.ui.MSPLayout.Divider;
import jlo.ioe.ui.MSPLayout.Node;
 ...
 * @author Hans Muller
 */
public class MSP extends JPanel {
    private AccessibleContext accessibleContext = null;
    private boolean continuousLayout = true;
    private DividerPainter dividerPainter = new DefaultDividerPainter();

    /**
     * Creates a MSP with it's LayoutManager set to 
     * to an empty MSPLayout.
     */
    public MSP() {
	super(new MSPLayout());
     ...
     * 
     * 
     * @return this MSP's layout manager
     * @see java.awt.Container#getLayout
     * @see #setModel
     */
    public final MSPLayout getMSPLayout() {
	return (MSPLayout)getLayout();
    }

    /** 
     * A convenience method that sets the MSPLayout model.
     * Equivalent to <code>getMSPLayout.setModel(model)</code>
     * 
     * @param model the root of the MSPLayout model
     * @see #getMSPLayout
     * @see MSPLayout#setModel
     */
    public final void setModel(Node model) {
	getMSPLayout().setModel(model);
    }

    /** 
     * A convenience method that sets the MSPLayout dividerSize
     * property. Equivalent to 
     * <code>getMSPLayout().setDividerSize(newDividerSize)</code>.
     * 
     * @param dividerSize the value of the dividerSize property
     * @see #getMSPLayout
     * @see MSPLayout#setDividerSize
     */
    public final void setDividerSize(int dividerSize) {
	getMSPLayout().setDividerSize(dividerSize);
    }
     ...
     *
     * @see MSP#getDividerPainter
     * @see MSP#setDividerPainter
     */
    ...
    /**
     * 
     * The DividerPainter that's used to paint Dividers on this MSP.
     ...
     * MSP.  The default DividerPainter only draws
     ...
     * the MSP's children have been rendered so that 
    ...
    private MSPLayout.Divider dragDivider = null;
	...
	MSPLayout msl = getMSPLayout();
	MSPLayout.Divider divider = msl.dividerAt(mx, my);
	if (divider != null) {
	    MSPLayout.Node prevNode = divider.previousSibling();
	    MSPLayout.Node nextNode = divider.nextSibling();
		...
		oldFloatingDividers = getMSPLayout().getFloatingDividers();
		getMSPLayout().setFloatingDividers(false);
		dragUnderway = true;
	    }
	}
	else {
	...
	if (dragUnderway) {
	    ...
	    getMSPLayout().setFloatingDividers(oldFloatingDividers);
	...
	if (show) {
	    MSPLayout.Divider divider = getMSPLayout().dividerAt(x, y);
	    if (divider != null) {
